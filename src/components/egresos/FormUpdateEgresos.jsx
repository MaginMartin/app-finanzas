...
const FormUpdateEgresos = ({idEgreso,dato,ocultarForm}) => {


    const { onInputChange,monto,titulo,descripcion,tipo,fechaInicio,cargarDatoEgresos} = useDataUpdateEgresos({
        idEgreso:idEgreso,
        monto:dato.monto,
          titulo: dato.titulo ,
    ...
    <TextField id="monto" label="Monto" type="number" placeholder='100000' 
      ...
      <TextField id="date" label="Fecha" type="date" 
        ...
        value={fechaInicio}  name="fechaInicio"  onChange={onInputChange}  InputLabelProps={{
          ...
          <Box sx={{ p: 1 }}>
      <Button sx={{ mr: 1 }}  variant="contained" size="medium"   onClick={()=>{cargarDatoEgresos();ocultarForm()}} >Modificar egreso</Button>
...
export default FormUpdateEgresos